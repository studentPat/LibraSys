...
3. Set the complete `ConnectionStrings__LibraryDatabase` value (including the password and TLS certificate settings) through environment configuration or a secret manager; the checked-in settings intentionally contain no password. Set `AllowedHosts` to deployed API hostnames, then run `dotnet run --project src/LibraSys.Api`.
