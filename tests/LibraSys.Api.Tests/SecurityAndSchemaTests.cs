...
public sealed class SecurityAndSchemaTests
{
    ...
    public void Initial_schema_contains_transactional_constraints()
    {
        ...
        Assert.Contains("/api/reservations", frontend);
        Assert.Contains("/api/circulation/borrow", frontend);
        Assert.Contains("/api/circulation/return", frontend);
        ...
        Assert.Contains("\"fines.create\"", File.ReadAllText(Path.Combine(root, "src", "LibraSys.Api", "Program.cs")));
        Assert.Contains("\"circulation.reserve\"", File.ReadAllText(Path.Combine(root, "src", "LibraSys.Api", "Program.cs")));
        ...
        Assert.Contains("/api/members/{memberId:long}/account", source);
        Assert.Contains("members.account.create", source);
        Assert.Contains("request.Password.Length < 12", source);
        Assert.Contains("WriteAuditAsync(connection, tx, actorUserId, \"catalog.book.create\"", source);
        Assert.Contains("WriteAuditAsync(connection, tx, actorUserId, \"members.create\"", source);
        var tuning = File.ReadAllText(Path.Combine(root, "database", "operations", "query-tuning.sql"));
